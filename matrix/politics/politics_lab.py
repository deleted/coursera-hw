...
def create_voting_dict():
    ...
    for line in voting_data:
        line = line.split(' ')
        d[line[0]] = [ int(r) for r in line[3:] ]
    return d
...
def most_similar(sen, voting_dict):
    ...
    """
    comparisions = [ (policy_compare(sen, sen_b, voting_dict), sen_b) for sen_b in voting_dict.keys() if sen_b != sen ]
    comparisions.sort()
    return comparisions[-1][1]
...
democrats = set( d[0] for d in (l.split(' ') for l in voting_data) if d[1] == 'D' )
avg_dems = sorted(democrats, key=lambda d: find_average_similarity(d, democrats, voting_dict))
most_average_Democrat = avg_dems[-1]
...
def find_average_record(sen_set, voting_dict):
    ...
    for i in range(len(votes[0])):
        sums.append( sum(v[i] for v in votes) )
    count = len(sen_set)
...
def bitter_rivals(voting_dict):
    ...
    import itertools
    results = []
    names = voting_dict.keys()
    for pair in itertools.combinations(names, 2):
        results.append( (policy_compare(pair[0], pair[1], voting_dict), pair) )
